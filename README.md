...
- Windows 新手推荐：下载并双击 `ZabbixOneClick-v1.0.4.exe`
- 完整文件包：解压 `release\ZabbixOneClick-v1.0.4.zip`
...
```text
ZabbixOneClick-v1.0.4.exe  单文件 GUI 版，内置 compose 和所有脚本，推荐发给新手
```

`ZabbixOneClick-v1.0.4.exe` 第一次运行时会自动解包到：
...
```text
dist\ZabbixOneClick-v1.0.4.exe
release\ZabbixOneClick-v1.0.4.exe
```
...
注意：EXE 只封装启动控制逻辑，不会把 Docker、MySQL、Zabbix 镜像塞进 EXE。首次运行仍会通过 Docker 下载官方镜像。交付给别人时推荐直接发送 `release\ZabbixOneClick-v1.0.4.exe`。
...
release\ZabbixOneClick-v1.0.4.exe
release\ZabbixOneClick-v1.0.4\
release\ZabbixOneClick-v1.0.4.zip
```

发给新手用户时，推荐直接给 `release\ZabbixOneClick-v1.0.4.exe`。
...
```text
ZabbixOneClick-v1.0.4.exe -> 修改配置
ZabbixOneClick-v1.0.4.exe -> 修改账号密码
```
...
```text
ZabbixOneClick-v1.0.4.exe -> 健康检查
ZabbixOneClick-v1.0.4.exe -> 问题修复
```
...
```text
ZabbixOneClick-v1.0.4.exe -> 备份数据库
```
